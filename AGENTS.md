...
- 调研类 `report.spec.md` 和 `*-slides.html` 只陈述可由来源直接核验的事实，包括项目定义、公开能力、结构、支持语言、用途、状态和仓库信息。
- 禁止由 AI 生成总结、推断、趋势判断、优劣判断、选型建议、启示、意义或行动建议。
- 分类与横向表格只用于组织事实，不得借分类结果推导未被来源直接陈述的结论。
- 确需呈现推断性内容时，必须由人工明确提供，并在材料中标注为人工观点。
- 删除口号式结尾、重复摘要和不增加事实的信息，按类别直接介绍调研对象，优先保留名称、来源、层级、语言覆盖、主要用途和可追溯链接。
