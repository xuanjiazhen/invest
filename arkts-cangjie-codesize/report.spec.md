...
该差分仍包含仓颉 `@State<String>` 生成的公共状态管理展开骨架，因此不作为剔除宏公共代码后的总对比结果。

### 3.4 排除 UI 宏公共展开骨架后的总对比

分层构建测得一个 `@State<String>` 触发的公共状态管理展开骨架为 6,208 B，数据位于 `team_interop/analysis/minimal-page-codesize-macro-spike/macro-measurement.json:17-26`。从仓颉应用壳差分中继续剔除该公共骨架：

| 项目 | 应用壳差分 | UI 宏公共骨架 | 总对比净业务代码 |
|---|---:|---:|---:|
| ArkTS `modules.abc` | 1,576 B | — | 1,576 B |
| 仓颉 strip `.so`，Full LTO | 11,936 B | -6,208 B | 5,728 B |

计算：

- 仓颉/ArkTS：`5,728 / 1,576 = 3.6345×`
- 绝对净增量：`5,728 - 1,576 = 4,152 B`
- 劣化率：`(5,728 - 1,576) / 1,576 × 100% = 263.45%`
...
完整功能页相对空 Row 应用壳的差分为 11,936 B，分解如下：
...
总对比剔除 6,208 B 公共状态管理骨架后，仓颉净业务代码为 `11,936 - 6,208 = 5,728 B`；该值用于第 3.4 节的 263.45% 劣化率。
...
1. ArkTS ABC 与仓颉 ELF 属于不同可执行交付格式；263.45% 表示剔除应用壳和仓颉 UI 宏公共状态管理骨架后的净业务交付尺寸差异，不表示逐指令编译效率。
