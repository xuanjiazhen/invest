...
> 覆盖范围：Kotlin/Android、Swift/iOS、ArkTS/HarmonyOS 资料部署模式
...
> **前提**：仓颉标准库 API 参考部署于 developer.huawei.com，与 ArkTS API 参考同级。以下聚焦**非标准库部分**（开发指南、工具链指南、实践、FAQ 等）。
...
4. **API 参考已在平台站**：标准库 API 参考部署于 developer.huawei.com，开发指南的部署需与 API 参考形成一致的查阅体验。
