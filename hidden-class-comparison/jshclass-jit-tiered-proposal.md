...
在 ArkVM 中，`JSHClass` 并不是用户代码里显式声明的对象类型，而是 VM 为“每一种对象形状（shape）”维护的一张内部布局描述表。这里的 `shape` 指的是对象当前属性布局的内部身份标识：同一类对象如果具备相同的属性集合、同样的属性偏移和相同的可枚举/可写特征，VM 会认为它们共享同一个 `shape`。`JSHClass` 正是这个 `shape` 对应的描述符。

它负责记录对象当前有哪些属性、这些属性在实例对象中的偏移位置、是否可枚举、是否可写，以及后续属性新增时的 transition 方向。
