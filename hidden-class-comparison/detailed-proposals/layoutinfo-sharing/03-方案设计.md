...
LayoutInfo 是 JSHClass 的属性描述表（物理格式与读写现状见 01-背景 §2–§5）。transition 过程中的防御性复制使**完整物理内容相同的 LayoutInfo 以不同对象大量共存**：TOP13 全应用 v2 严格内容模型下，735,747 个 distinct Layout 合计 shallow 109.25 MiB，完整槽重复折算 69.64 MiB；快手前台单应用为 8.17 MiB（shallow 12.96 MiB 的 63%）。防复制的原因是 LayoutInfo 存在原地写入点（TrackType/Attr/PGO 标志等，01-背景 §4），父子共享数组会互相串扰。
...
| 问题 | 影响 |
|------|------|
| 无内容寻址，相同内容多对象并存 | TOP13 严格重复 69.64 MiB；快手前台 8.17 MiB |
...
TOP13 全应用严格结果：合计 shallow 109.25 MiB，完整槽模型覆盖 99.946% 对象 / 99.697% 字节；毛重复 69.64 MiB，16 B 表成本 4.03 MiB，条件净 65.61 MiB（占现状 shallow 60.1%）。13 个应用均为正；逐应用条件净 1.09–12.34 MiB，明细与输入哈希见 [07-TOP13现状与收益验证.md](07-TOP13现状与收益验证.md) 和 [06-收益数据.csv](06-收益数据.csv)。
...
| R9 | snapshot 条件净 shallow 被当成实现后物理收益 | 中 | 中 | shallow/Region/PSS 分列；表头、tombstone、锁、COW 不隐去 | clean A/B 验证 Layout shallow、Region used/committed、PSS；未达门槛不放行 |
...
| 性能 | ⭐⭐⭐ | TOP13 条件净容量 65.61 MiB；快手前台/后台 7.74/6.34 MiB；COW/物理收益待实测关闭 |
| 风险可控 | ⭐⭐⭐⭐ | 关闭证据逐项可验证 |
| 可回滚 | ⭐⭐⭐⭐⭐ | 双开关，默认关闭 |
