...
| 字段外存索引表 A+B2（备选，§4.7） | 通用构建 24 B（`8N`），IR 可偏移访问 | 内联 4 B 索引 + 表基础设施 + profiler 记录转 runtime 调用；≤6G 档劣于裁槽（16 B） |

### 4.7 备选路线：字段外存（通用构建统一 16 B）

**定位**：裁槽两阶段只作用于能力闭包档位；本备选把 `MachineCode`（索引表）与 `Handle`（堆外会话表）外存，使**通用构建（JIT+PGO 全开）得到 24 B cell（8N）**。≤6G 档裁槽可达 16 B、更优且无需表基础设施，维持首选；本路线仅当「通用构建也需收缩 cell」成为需求时立项，与裁槽共用能力闭包体系、构建期二选一，同镜像不混布。
...
**表 A：CodeIndexTable（MachineCode 外存，索引表设计）**

```text
cell 内联                     per-VM 索引表（native，free-list 分配）
┌─────────────────┐          ┌──────────────────────────────┐
│ header 8 B       │          │ Row[idx] { weak MachineCode*; │ ←16 B/行
│ Value  8 B       │          │             uint8 state }     │
│ ExtIdx 4 B + pad │──idx──►  │ base + idx × rowSize          │
└─────────────────┘          └──────────────────────────────┘
访问链（可编译为 IR 偏移指令）：
  load cell.ExtIdx（固定偏移）→ add base + idx×16 → load 行字段
  两次依赖 load、零分支、零哈希、读侧无锁
```

- **为什么是索引表不是哈希表**（人工评审意见）：哈希查找无法被 JIT IR 编译为偏移指令序列；索引表使任何当前或未来的生成代码消费者（tiering 检查、诊断 stub 等）都能以固定指令数访问——当前三触点虽均为 C++，索引化是设计稳健性选择；
- **代价与收益修正**：cell 需内联 4 B `ExtIdx`（+4 B 对齐），通用构建 cell 为 **24 B（8N）**而非哈希方案的 16 B——IR 可编译性与内联索引互为代价；
- **GC 集成（leaptiering 同款）**：cell visitor 标记所属行（活 cell 保行）；行内 weak MachineCode 走既有 weak 处理；死 cell 的行不被标记 → 弱值清理后回 free-list——比哈希双弱清理更简单，且无 rehash/tombstone；
- **并发**：行分配走 free-list CAS；安装（JIT 线程）以 release 写行字段；读侧无锁；
- **API 与现有触点一一对应**：`AcquireRow(cell)`（安装）、`ClearRow(cell)`（deopt 复位的写 Hole 动作改为清行弱值）、`LoadRow(cell)`（诊断）；
- **容量**：人口 = 被 JIT 编译的定义点（10³~10⁵），表本体 ≤ 2 MiB；
- **业界对位**：V8 `JSDispatchTable`（内联 handle + 行表 + 标记驱动回收）同构。
...
**收益与成本（对齐 02 §2.1 口径）**：通用构建 32→24 B = `8N`：前台 543,520 B（0.518 MiB）、后台 461,712 B（0.440 MiB）；成本 = 表 A ≤2 MiB + 会话表（会话期临时）+ profiler 记录路径的 runtime 调用（事件级频率，DEFINECLASS 密集场景实测列入放行门槛）。若未来证明无任何 IR 消费者且接受 C++-only 访问，可评估免内联索引的哈希变体（16 B/16N），当前不采纳（人工评审意见：哈希不可被 IR 偏移指令访问）。
...
阶段一 JIT-free 24 B 裁槽可独立进入实现；阶段二 16 B 档等待显式 PGO-free 产品决策。两阶段只缩小 cell 尺寸，不改变数量、创建时机或共享语义。前台快照累计收益分别为 0.518 MiB 和 1.037 MiB，后台 full-GC 分别为 0.440 MiB 和 0.881 MiB。字段外存备选路线（§4.7，索引表 + B2 会话表）为通用构建提供 24 B（8N）选项，访问链可编译为 IR 偏移指令；≤6G 档劣于裁槽（16 B），维持裁槽首选。
